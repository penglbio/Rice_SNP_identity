...
rule fastqc_raw_PE:
	...
	shell:
		'fastqc -t 20 -o fastqc/raw {input}'
...
rule trimmomatic_PE:
	...
	shell:
		'trimmomatic PE -threads 20 -phred33 {input.r1} {input.r2} {output.r1_paired} {output.r1_unpaired} {output.r2_paired} {output.r2_unpaired} ILLUMINACLIP:{params.adapter}:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36'
...
rule fastqc_clean_PE:
	...
	shell:
		'fastqc -t 20 -o fastqc/clean {input}'

rule fastqc_stat_PE:
	input:
		['fastqc/raw/{sample}_1_fastqc.html'.format(sample=x) for x in config['samples']],
		['fastqc/raw/{sample}_2_fastqc.html'.format(sample=x) for x in config['samples']],
...
rule map_to_genome_PE:
	...
	shell:
		'{params.conda}/bowtie2 -p {params.cpu} -x {params.index} -1 {input.r1} -2 {input.r2} |samtools view -@ {params.cpu} -Shub|samtools  sort -@ {params.cpu} --output-fmt BAM - -T {params.prefix} -o {output.bam}'
...
rule bam_rmdup:
	...
	params:
		conda=config['conda_path']
	shell:
		'{params.conda}/samtools rmdup {input.bam} {output.rmdup}'
...
rule bam_mpileup:
	...
	params:
		...
		conda=config['conda_path']
	shell:
		'{params.conda}/samtools mpileup -t DP,SP -Eg -f {params.genome} {input.bam}|bcftools call -mv > {output.vcf}'
...
rule vcf_filter:
	...
	params:
		conda=config['conda_path']
	shell:
		'{params.conda}/bcftools view {input} | vcfutils.pl varFilter -d 20 -Q 20 - >{output}'
...
rule vcf_index:
	...
	params:
		conda=config['conda_path']
	shell:
		'{params.conda}/bcftools index {input} -o {output}'
rule bam_qc:
	input:
		bam = 'bam/{sample}.sort.bam'
	output:
